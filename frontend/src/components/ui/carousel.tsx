...
interface SlideProps {
  ...
  openModalDetails: (src: string) => void;
}

const Slide = ({ slide, index, current, handleSlideClick, openModalDetails }: SlideProps) => {
...
export function Carousel({ slides }: CarouselProps) {
  ...
  function openModalDetails(imageSrc: string) {
    // Modal pass karvu padse with the index details
    setModalOpen(true)
...
function Modal({ imgSrc, onClose }: { imgSrc: string; onClose: () => void }) {
  return (
    <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50">
      <div className="relative bg-white p-4 rounded shadow-lg">
        <button
          className="absolute text-3xl top-2 right-2 text-black"
          onClick={onClose}
        >
          &times;
        </button>
        <img src={imgSrc} alt="Modal" className="max-w-[80vw] max-h-[80vh]" />
      </div>
    </div>
  );
}
